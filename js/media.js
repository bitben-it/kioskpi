...
function updateVideosGrid(videos) {
    if (!dom.videosGrid) {
        console.warn('videosGrid element not found');
        return;
    }
    ...
    videos.forEach((video, index) => {
        ...
        if (!video || !video.src || !video.label) {
        ...
        try {
            ...
            // Disable download (may not be supported in older browsers)
            if (videoEl.controlsList !== undefined) {
            ...
            console.error('Error creating video element:', error, video);
        }
    });
}
...
function updateAudiosGrid(audios) {
    ...
    audios.forEach((audio, index) => {
        ...
        if (!audio || !audio.src || !audio.label) {
            console.warn('Invalid audio object at index', index, audio);
            return;
        }
        ...
        try {
            ...
            // Disable download (may not be supported in older browsers)
            if (audioEl.controlsList !== undefined) {
            ...
            console.error('Error creating audio element:', error, audio);
        }
    });
